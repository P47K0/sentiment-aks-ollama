...
def test_detect_language_no_text(monkeypatch, client):
     monkeypatch.setenv("LANGUAGE_DETECTION_ENABLED", "true")
     response = client.post('/detect-language', json={})
     assert response.status_code == 400
     assert response.json == {"error": "No text provided"}

def test_detect_language_empty_text(monkeypatch, client):
     monkeypatch.setenv("LANGUAGE_DETECTION_ENABLED", "true")
